...
async def authenticate(*, email, passwd):
    ...
    r = web.Response()
    r.set_cookie(COOKIE_NAME, user2cookie(user, 864000), max_age=864000, httponly=True)
    user.passwd = '******'
...
_RE_EMAIL = re.compile(r'(^[a-z0-9_.+-]+@[a-z0-9-]+\.[a-z0-9-.]+$)')
...
async def api_create_blog(request, *, name, summary, content):
    ...
    logging.info(len(request.__user__.image))
    logging.info(request.__user__.image)
